...
parser.add_argument('--save_every_step', type=int, default=20000, help='save checkpoint every num step')
...
def inference(images, mode='train'):
    ...
    # flatten the norm3
    flatten = tf.reshape(norm3, shape=[batch_size, -1])
    ...
    with tf.variable_scope('softmax_linear'):
    	logits = fc_relu(drop2, [384, 5], [5], None, None)
...
def main():
	...
	with tf.device('/cpu:0'):
		example_batch = tfRecord_input_pipeline.input_pipeline(batch_size=FLAGS.batch_size)
	...
	try:
	    for i in range(FLAGS.train_step):
	        if not coord.should_stop():
	            ...
	            _, merged_sum = sess.run([train_op, merged])
	            if i % 500 == 0:
